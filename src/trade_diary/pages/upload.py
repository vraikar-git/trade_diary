...
def upload_file(contents, file_name, date_format):
    ...
    for index, row in df_agg_trades.iterrows():
        try :
            ...
            logging.error(f"Error inserting trade {row['symbol']} {row['entry_date']}: {e}")
            not_inserted_trades.append((index, row['symbol'], row['entry_date'], str(e)))
    ...
    trades = pd.DataFrame(inserted_trades, columns=['trade_id', 'index', 'symbol', 'entry_date'])

    df_trades = trades.merge(df, on=['symbol', 'entry_date'], how='inner').query("exit_date.notnull()")

    if df_trades.empty:
        logging.warning("No Closed trade found.")
        return f"File uploaded successfully! {len(inserted_trades)} trades inserted."

    df_exits_agg = df_trades.groupby(['trade_id', 'exit_date']).agg(
        {
            'exit_price': 'first',
            'quantity': 'sum',
        }
