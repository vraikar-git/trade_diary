from dash import Dash, callback_context, html, dcc, callback, Output, Input, State, no_update, set_props
import dash

from src.trade_diary.validate import *
from src.trade_diary.db_interface import *
from src.trade_diary.utility_functions import add_additional_columns, extract_financial_year
...
layout =  dbc.Row([
    ...
    # extras
    db_update_store,
    dcc.ConfirmDialog( id = 'error-dialog',message='',displayed=False,),
    info_dialog,
    entry_dialog,
    exit_dialog,
    pyramid_dialog,
...
@callback(
    ...
    Input('trades-table','selectedRows'), prevent_initial_call=True
)
...
@callback(
    ...
    Input('add-position','n_clicks'), prevent_initial_call=True
)
...
@callback(
        ...
        State('stop-loss', 'value')], prevent_initial_call=True )
def on_add_submit(n_clicks, symbol, entry_date, entry_price, entry_quantity, risk_percent, setup, entry_type, stop_loss):
...
State('trades-table', 'selectedRows'), prevent_initial_call=True
)
def on_exit_position(n_clicks, selectedRows):
    ...
    if selectedRows and n_clicks:
        if selectedRows[0]['total_open_position'] > 0 :
            set_props('exit-quantity', {'value' : selectedRows[0]['total_open_position']})
            return True
        else :
...
@callback(
    ...
    State('trades-table', 'selectedRows'), prevent_initial_call='initial_duplicate'
)
def on_exit_submit(n_clicks, exit_quantity, exit_price, exit_date, exit_type, selectedRows):
    if selectedRows and n_clicks:
...
def on_pyramid(n_clicks, selectedRows):
    logging.debug(f"on_pyramid: Pyramid Position Clicked with {selectedRows} and {n_clicks}")
    if selectedRows and n_clicks:
        if selectedRows[0]['total_open_position'] > 0 :
            return True
        else :
            set_props('info_dialog', {'is_open' : True})
            set_props('info_dialog_text', {'children' : 'Cannot Pyramid on Closed Trade'})
            return no_update
    return no_update
...
def on_pyramid_submit(n_clicks, entry_quantity, entry_price, entry_date, risk_percentage, stop_loss, selectedRows):
    logging.debug(f"on_pyramid_submit: Pyramid Submit Clicked with {selectedRows} and {n_clicks}")
    if selectedRows and n_clicks:
...
@callback(
            Output('info_dialog','is_open'),
            Input('info_dialog_close', 'n_clicks')
)
...
def add_position(symbol, entry_price, entry_quantity, entry_date, risk_percentage, setup, entry_type, stop_loss):
...
def exit_position(trade_id, total_open_position, exit_price, exit_quantity,exit_date, exit_type):
        error = validate_exit_position(total_open_position, exit_price, exit_quantity, exit_date)
        ...
        exit_id = insert_exit(trade_id=trade_id, exit_price=exit_price, quantity=exit_quantity, exit_date=edate, exit_type=exit_type)
...
def pyramid_position(trade_id, entry_price, entry_quantity, entry_date, risk_percentage, stop_loss):
        error = validate_pyramid_position(entry_price, entry_quantity, entry_date, risk_percentage, stop_loss)
