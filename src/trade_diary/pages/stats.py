...
from src.trade_diary.db_interface import get_all_financial_years, get_all_entries, get_all_exits, get_all_trades
...
def get_display_data(financial_year):
    ...
    entries['risked_amount'] = entries['quantity'] * (entries['entry_price'] - entries['stop_loss'])

    entries_agg = entries.groupby('trade_id').agg({
        'entry_id' : 'count',
        'entry_date' : 'min', 
        'entry_amount': 'sum',
        'quantity': 'sum',
    ...
    entries_agg['gross_pl'] = (entries_agg['exit_amount'] - entries_agg['entry_amount']).round(2)
    entries_agg['net_pl'] = (entries_agg['gross_pl'] - entries_agg['charges']).round(2)
    entries_agg['net_pl_percentage'] = ((entries_agg['net_pl'] / entries_agg['entry_amount']) * 100).round(2)
    entries_agg['gross_R'] = (entries_agg['net_pl'] / entries_agg['risked_amount']).round(2)
    entries_agg['net_R'] = (entries_agg['risk_percentage'] * entries_agg['gross_R']).round(2)
    ...
    exits_agg = exits.groupby('trade_id').agg(exit_count=('exit_id', 'count'), exit_date=('exit_date', 'max')).reset_index()

    trades = get_all_trades(financial_year=financial_year)
    trades['initial_entry_date'] = pd.to_datetime(trades['initial_entry_date'], format = '%Y-%m-%d')
    trades['i_entry_date'] = trades['initial_entry_date'].dt.date
    trade_month = trades['initial_entry_date'].dt.month
    conditions = [(trade_month >= 1) & (trade_month <= 3),
    ...
    trades['no_of_days_win'] = np.where(trades['win'] == 1, trades['no_of_days'], np.nan)
    trades['no_of_days_loss'] = np.where(trades['win'] == 0, trades['no_of_days'], np.nan)
    

    trades_disp_cols = [ 'symbol', 'i_entry_date', 'setup', 'financial_year', 'qtr',
           'risk_percentage','charges',  'gross_pl', 'net_pl', 'net_pl_percentage',
       'gross_R', 'net_R', 'win', 'no_of_days','no_of_days_win', 'no_of_days_loss']
    renamed_cols = {
        'symbol': 'Symbol',
    ...
    trades_display = trades[trades_disp_cols].sort_values(by='i_entry_date').rename(columns=renamed_cols)
    ...
    for name, grouper in groupers.items():
        display_df = trades.groupby(grouper).agg(
...
summary_year = dbc.Row(
    [
        html.H5('Yearly', style={'textAlign': 'center',  'marginTop': '20px', 'fontWeight': '500', 'fontSize': '1.3rem',}),
        html.Hr(),
        html.Div(
            id = "summary-tab-yearly",
...
summary_qtr = dbc.Row(
    [
        html.H5('Quarterly', style={'textAlign': 'center',  'marginTop': '20px', 'display': 'block', 'fontWeight': '500', 'fontSize': '1.3rem',}),
        html.Hr(),
        html.Div(
            id = "summary-tab-quarterly",
            className="table-responsive"
        )
    ],
)
...
summary_month = dbc.Row(
    [
        html.H5('Monthly', style={'textAlign': 'center',  'marginTop': '20px', 'display': 'block', 'fontWeight': '500', 'fontSize': '1.3rem',}),
        html.Hr(),
        html.Div(
            id = "summary-tab-monthly",
            className="table-responsive"
        )
    ],
)
...
summary_setup = dbc.Row(
    [
        html.H5('Set-Up', style={'textAlign': 'center',  'marginTop': '20px', 'display': 'block', 'fontWeight': '500', 'fontSize': '1.3rem',}),
        html.Hr(),
        html.Div(
            id = "summary-tab-setup",
            className="table-responsive"
        )
    ],
)
...
summary_trades = dbc.Row(
    [
        html.H5('Trades', style={'textAlign': 'center',  'marginTop': '20px', 'display': 'block', 'fontWeight': '500', 'fontSize': '1.3rem',}),
        html.Hr(),
        html.Div(
            "trsting",
            id = "summary-tab-trades",
            className="table-responsive"
    ...
    [
        ...
        dbc.Container([
            ...
            dcc.Checklist(
                [ 
                    {
                    "label" : html.Span("Show Trades", style={"font-size": '1.5rem', "padding-left": 10}),
                     "value": "yes",
                     }
                ],
                id='show-trades',
...
def update_summary_header(input_value, show_trades):
    ...
    return header, summary_tab_yearly, summary_tab_quarterly, summary_tab_monthly, summary_tab_setup, summary_trades, drop_down_options