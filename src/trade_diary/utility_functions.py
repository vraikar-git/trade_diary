...
def calculate_charges(entry_date, exit_date, buy_amount, sell_amount):
    ...
    brokerage = min(charges['brokerage'][trade_type] * buy_amount,20) + min(charges['brokerage'][trade_type] * sell_amount,20)
    stt = charges['stt'][trade_type] * sell_amount if trade_type == 'intraday' else charges['stt'][trade_type] * turnover
    transaction_tax = charges['transaction_tax'][trade_type] * buy_amount + charges['transaction_tax'][trade_type] * sell_amount 
    ...
    total_charges = brokerage + stt + transaction_tax + sebi_charges + stamp_duty + gst + dp_charges
...
def get_entry_adjustment_details(entries, exit_date, exit_quantity, exit_price):
    ...
    for index, row in entries.iterrows():
        ...
        adjust_entry_details.append((entry_id, remaining_quantity, exit_amount, old_charges, exit_price, considered_qty, total_charges))
